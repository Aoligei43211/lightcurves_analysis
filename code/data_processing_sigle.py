...
使用方法：
    get_sorted_data(file_path=None)  # 调用函数获取处理后的数据，可指定文件路径

参数：
    file_path: 可选，指定FITS文件路径

返回值：
    time_sorted, flux_sorted: 排序后的时间数组和通量数组

注意事项：
    函数名保持为get_sorted_data，以确保与其他文件的兼容性
"""
import numpy as np
import os
...
def get_sorted_data(file_path=None):
    """
    读取并处理单个FITS文件中的数据
    ...
    if not file_path.lower().endswith('.fits'):
        print(f"错误：文件不是FITS格式: {file_path}")
        return None, None
